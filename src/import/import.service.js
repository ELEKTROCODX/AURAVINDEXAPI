...
export const import_default_data = async () => {
    ...
    apiLogger.info("Importing default data...\n");
    
    for (const role of roles.roles) {
        const exists = await role_repository.filter_roles({name: role.name});
        if(exists.length == 0) {
            apiLogger.info("Creating role: " + role.name);
            await role_repository.create_role(role);   
        } else {
            apiLogger.warn("Role " + role.name + " already exists");
            
        }
    }
    apiLogger.info("Roles successfully imported.");

    for (const gender of genders.genders) {
        const exists = await gender_repository.filter_genders({name: gender.name});
        if(exists.length == 0) {
            apiLogger.info("Creating gender: " + gender.name);
            await gender_repository.create_gender(gender);   
        } else {
            apiLogger.warn("Gender " + gender.name + " already exists");
            
        }
    }
    apiLogger.info("Genders successfully imported.");
    ...
    for (const book_status of book_statuses.book_statuses) {
        ...
        if(exists.length == 0) {
            apiLogger.info("Creating book_status: " + book_status.book_status);
            await book_status_repository.create_book_status(book_status);   
        } else {
            apiLogger.warn("Book statuses " + book_status.book_status + " already exists");
            
        }
    }
    apiLogger.info("Book statuses successfully imported.");
    ...
    for (const book_collection of book_collections.book_collections) {
        ...
        if(exists.length == 0) {
            apiLogger.info("Creating book collection: " + book_collection.name);
            await book_collection_repository.create_book_collection(book_collection);   
        } else {
            apiLogger.warn("Book collection " + book_collection.name + " already exists");
            
        }
    }
    apiLogger.info("Book collections successfully imported.");
    ...
    for (const log_action of log_actions.log_actions) {
        ...
        if(exists.length == 0) {
            apiLogger.info("Creating log action: " + log_action.action_code);
            await log_action_repository.create_log_action(log_action);   
        } else {
            apiLogger.warn("Log action " + log_action.action_code + " already exists");
            
        }
    }
    apiLogger.info("Log actions successfully imported.");
    ...
    for (const loan_status of loan_statuses.loan_statuses) {
        ...
        if(exists.length == 0) {
            apiLogger.info("Creating loan status: " + loan_status.loan_status);
            await loan_status_repository.create_loan_status(loan_status);   
        } else {
            apiLogger.warn("Loan status " + loan_status.loan_status + " already exists");
            
        }
    }
    apiLogger.info("Loan statuses successfully imported.");
    ...
    for (const plan_status of plan_statuses.plan_statuses) {
        ...
        if(exists.length == 0) {
            apiLogger.info("Creating plan status: " + plan_status.plan_status);
            await plan_status_repository.create_plan_status(plan_status);   
        } else {
            apiLogger.warn("Plan status " + plan_status.plan_status + " already exists");
            
        }
    }
    apiLogger.info("Plan statuses successfully imported.");

    for (const plan of plans.plans) {
        const exists = await plan_repository.filter_plans({name: plan.name});
        if(exists.length == 0) {
            apiLogger.info("Creating plan: " + plan.name);
            await plan_repository.create_plan(plan);   
        } else {
            apiLogger.warn("Plan " + plan.name + " already exists");
    ...
    for (const user of users.users) {
        ...
        if(exists.length == 0) {
            ...
            apiLogger.info("Creating user: " + user.username);
            ...
            apiLogger.warn("User " + user.username + " already exists");
            
        }
    }
    apiLogger.info("Users successfully imported.");
