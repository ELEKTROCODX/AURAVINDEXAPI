...
export const find_all_book_lists = async (skip, limit) => {
    return await book_list_model.book_list.find().skip(skip).limit(limit).populate('title owner books');
}

// Fetch with filters
export const filter_book_lists = async (filter, skip, limit) => {
    return await book_list_model.book_list.find(filter).skip(skip).limit(limit).populate('title owner books');
}
...
export const find_book_list_by_id = async (id) => {
    return await book_list_model.book_list.findById(id).populate('title owner books') || null;
}
