 ...
 * @returns {void}
 * 
 * @throws {ObjectAlreadyExists} - If the user already exists with the provided username or email.
 */
export const register = async (req, res) => {
    try {
        const {username, name, last_name, email, biography, gender, favorite_book, birthdate, password} = req.body;
        const user_img = req.file ? `/images/users/${req.file.filename}` : app_config.DEFAULT_USER_IMG_PATH;
        await auth_service.register(username, name, last_name, email, biography, gender, favorite_book, birthdate, user_img, password);
        const user_data = await user_service.filter_users('username', username, 1, 1);
        await audit_log_service.create_new_audit_log(user_data.data[0]._id, app_config.PERMISSIONS.SIGNUP, username);
...
export const login = async (req, res) => {
    try {
        ...
        await audit_log_service.create_new_audit_log(user_data.data[0]._id, app_config.PERMISSIONS.SIGNIN, user_data.data[0].username);
...
export const request_password_reset = async (req, res) => {
    try {
        ...
        const user_data = await user_service.filter_users('email', email, 1, 1);
        await audit_log_service.create_new_audit_log(user_data.data[0]._id, app_config.PERMISSIONS.REQUEST_PASSWORD_RESET, user_data.data[0].username);
        if(response) {
...
export const reset_password = async (req, res) => {
    try {
        ...
        await audit_log_service.create_new_audit_log(password_reset._id, app_config.PERMISSIONS.RESET_PASSWORD, password_reset.username);
