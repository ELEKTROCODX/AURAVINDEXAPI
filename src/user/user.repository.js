...
export const find_all_users = async (skip, limit) => {
    return await user_model.user.find().skip(skip).limit(limit).populate('username favorite_book gender role');
}

// Fetch with filters
export const filter_users = async (filter, skip, limit) => {
    return await user_model.user.find(filter).skip(skip).limit(limit).populate('username favorite_book gender role');
}
...
export const find_user_by_id = async (id) => {
    return await user_model.user.findById(id).populate('username favorite_book gender role') || null;
}
